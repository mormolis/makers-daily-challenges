...
k={["Basingstoke and Deane", "Tadley South"]=>
  {:count=>1,
   :average=>94000,
   :transactions=>
    [[94000,
...
def convert_hash(hash)
  s=""
  hash.each {|k,v| s+="A house was sold in #{k[0]}, #{k[1]} for #{v[:average]} on #{v[:transactions][0][2].strftime('%D')}\n"}
  s
end

def search_hash (hash, d_or_w)
  s=""
  hash.each do |k, v|
    if k[0] == d_or_w || k[1] == d_or_w
      s+= "A house was sold in #{k[0]}, #{k[1]} for #{v[:average]} on #{v[:transactions][0][2].strftime('%D')}\n"
    end
  end
  if s.empty?
    puts "no results on #{d_or_w}"
  end
  s
end

puts search_hash(k,"Bishop's Lydeard")
puts search_hash(k,"ydeard")
